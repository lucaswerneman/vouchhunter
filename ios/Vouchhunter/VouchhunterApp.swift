        ...
        Group {
            if session.loggedIn {
                TabView {
                    ...
                    NavigationStack { List { Section("Ditt konto") { Button("Logga ut",role:.destructive){Task{await session.logout()}} } }.navigationTitle("Profil") }.tabItem{Label("Profil",systemImage:"person.crop.circle")}
                }
            } else { LoginView(session:session) }
        }
        .sheet(item:$linkedCampaign){c in NavigationStack{HuntView(campaign:c)}}
        .onOpenURL{url in
            guard let id=url.pathComponents.last,id.range(of:"^[a-f0-9]{24}$",options:.regularExpression) != nil else{return}
            Task { do { linkedCampaign=try await API.shared.request("/campaigns/"+id) } catch {linkError=error.localizedDescription} }
        }
...
struct LoginView: View {
    ...
    private func submit() async {
        ...
        do {
            let response:LoginResponse=try await API.shared.request(register ? "/register" : "/login",body:["email":email,"password":password,"name":name,"native":true])
            try Keychain.save(response.token);session.loggedIn=true
...
struct HuntView: View {
        ...
        ScrollView {
            VStack(alignment:.leading,spacing:20){
                ...
                if let hunt,hunt.completed != nil {Label("Du är klar! Din voucher finns i plånboken.",systemImage:"checkmark.seal.fill").foregroundStyle(.green)}
                    ...
                    Button("Starta jakten"){Task{await start()}}.buttonStyle(.borderedProminent).disabled(busy)
                } else if let hunt {Text("Reserverad till \(Date(timeIntervalSince1970:hunt.expires).formatted(date:.omitted,time:.shortened))").font(.footnote)}
                Map {
                    UserAnnotation()
                    ForEach(campaign.stops){s in Marker(s.name,coordinate:.init(latitude:s.lat,longitude:s.lon)).tint(hunt?.collected.contains(s.id)==true ? .gray : .green)}
                }.frame(height:240).clipShape(RoundedRectangle(cornerRadius:16))
                Text("Platser att upptäcka").font(.title2.bold())
                ForEach(campaign.stops){s in HStack{VStack(alignment:.leading){Text(s.name).font(.headline);if let loc=location.location {Text("\(Int(loc.distance(from:CLLocation(latitude:s.lat,longitude:s.lon)))) m bort").foregroundStyle(.secondary).font(.footnote)}};Spacer();if hunt?.collected.contains(s.id)==true{Image(systemName:"checkmark.circle.fill").foregroundStyle(.green)}else{Button("Samla"){selected=s}.buttonStyle(.bordered).disabled(!canCollect(s))}}.padding(.vertical,10)}
    ...
    private func canCollect(_ stop:Stop)->Bool {
        ...
        return loc.distance(from:CLLocation(latitude:stop.lat,longitude:stop.lon))<=Double(stop.radius)
    }
    private func start() async {busy=true;defer{busy=false};do{hunt=try await API.shared.request("/hunts/"+campaign.id+"/start",body:[:]);error=""}catch{self.error=error.localizedDescription}}
    private func collect(_ stop:Stop) async throws {
        location.start()
        var payload=try location.payload();payload["stop_id"]=stop.id
