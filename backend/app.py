...
def text_field(data, key, minimum=1, maximum=200):
    value = data.get(key)
    require(isinstance(value, str) and minimum <= len(value.strip()) <= maximum, f'Kontrollera fältet {key}.')
    return value.strip()
def number(value, low, high):
    require(isinstance(value, (int, float)) and not isinstance(value, bool) and math.isfinite(value) and low <= value <= high, 'Ogiltigt numeriskt värde.')
    return value
...
def distance(a,b,c,d):
    ...
    v=math.sin((p2-p1)/2)**2+math.cos(p1)*math.cos(p2)*math.sin(math.radians(d-b)/2)**2
...
class Store:
    ...
    def audit(self,db,actor,action,entity):
        db.execute('INSERT INTO audit(actor,action,entity,created) VALUES(?,?,?,?)',(actor,action,entity,now()))
    def user(self,db,token):
        row=db.execute('SELECT u.id,u.name,u.email FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token=? AND s.expires>? AND s.revoked=0',(hash_token(token),now())).fetchone()
    ...
    def hunt(self,db,user,cid):
        ...
        result=dict(h)
        result['collected']=[r[0] for r in db.execute('SELECT stop_id FROM collections WHERE hunt_id=?',(h['id'],))]
    ...
    def start(self,db,user,cid):
        ...
        if existing and (existing['completed'] or existing['expires']>now()): return existing
        require(c['status']=='active' and c['starts']<=now()<c['ends'],'Kampanjen är inte öppen.',409)
        taken=db.execute('SELECT count(*) FROM hunts WHERE campaign_id=? AND (completed IS NOT NULL OR expires>?)',(cid,now())).fetchone()[0]
        require(taken<c['capacity'],'Alla belöningar är reserverade just nu.',409)
        expires=min(now()+3600,c['ends'])
        if existing:
            db.execute('UPDATE hunts SET expires=? WHERE id=?',(expires,existing['id']))
        else: db.execute('INSERT INTO hunts(id,user_id,campaign_id,expires) VALUES(?,?,?,?)',(uid(),user,cid,expires))
    ...
    def collect(self,db,user,cid,data):
        ...
        if stop['id'] in h['collected']: return h
        require(not h['completed'] and h['expires']>now(),'Reservationen har gått ut. Starta jakten igen.',409)
        require(c['status']=='active' and c['starts']<=now()<c['ends'],'Insamlingen är pausad eller avslutad.',409)
        lat=number(data.get('lat'),-90,90);lon=number(data.get('lon'),-180,180)
        accuracy=number(data.get('accuracy'),0,35)
        captured=number(data.get('captured_at'),now()-45,now()+10)
        require(distance(lat,lon,stop['lat'],stop['lon'])<=stop['radius'],'Gå närmare platsen för att samla objektet.',409)
        db.execute('INSERT INTO collections VALUES(?,?,?)',(h['id'],stop['id'],now()))
        count=db.execute('SELECT count(*) FROM collections WHERE hunt_id=?',(h['id'],)).fetchone()[0]
        if count>=c['target']:
            db.execute('UPDATE hunts SET completed=? WHERE id=?',(now(),h['id']))
            db.execute('INSERT INTO vouchers(id,hunt_id,code,expires) VALUES(?,?,?,?)',(uid(),h['id'],secrets.token_urlsafe(24),now()+c['voucher_days']*86400))
            self.audit(db,user,'voucher.issued',h['id'])
        return self.hunt(db,user,cid)
    def redeem(self,db,user,code,commit):
        row=db.execute('SELECT v.*,c.org_id,c.title,c.reward,c.venue FROM vouchers v JOIN hunts h ON h.id=v.hunt_id JOIN campaigns c ON c.id=h.campaign_id WHERE v.code=?',(code,)).fetchone()
        ...
        if commit:
            db.execute('UPDATE vouchers SET redeemed=?,redeemed_by=? WHERE id=? AND redeemed IS NULL',(now(),user,row['id']))
            self.audit(db,user,'voucher.redeemed',row['id'])
        return {'id':row['id'],'title':row['title'],'reward':row['reward'],'venue':row['venue'],'redeemed':commit}

class App:
    def __init__(self,path=None): self.store=Store(path or os.environ.get('DATABASE_PATH',ROOT/'data/vouchhunter.sqlite3')); self.attempts={}
    def __call__(self,e,start):
        ...
        try:
            ...
            start('200 OK',headers);return [body]
        except Problem as ex:
            start(f'{ex.status} Error',headers);return [json.dumps({'error':ex.message}).encode()]
        except (ValueError,TypeError,KeyError):
            start('400 Bad Request',headers);return [b'{"error":"Ogiltig begaran."}']
        except Exception:
            import traceback;traceback.print_exc()
            start('500 Internal Server Error',headers);return [b'{"error":"Ett serverfel uppstod. Forsok igen."}']
    def static(self,path,start):
        file=(ROOT/'web'/path.lstrip('/')).resolve()
        if path=='/' or not Path(path).suffix: file=ROOT/'web/index.html'
        ...
        with self.store.transaction() as db:
            if path in ('/api/register','/api/login') and method=='POST':
                ...
                if path=='/api/register':
                    require(not row,'Kontot kunde inte skapas med dessa uppgifter.',409)
                    user=uid();db.execute('INSERT INTO users VALUES(?,?,?,?,?)',(user,email,password_hash(pw),text_field(data,'name',1,100),now()))
                    company=data.get('company','').strip()
                    if company:
                        require(len(company)<=100,'Företagsnamnet är för långt.')
                        org=uid();db.execute('INSERT INTO organizations VALUES(?,?,?)',(org,company,now()));db.execute('INSERT INTO memberships VALUES(?,?,?)',(user,org,'owner'))
                else:
                    calculated=password_hash(pw,row['password'].split(':')[0] if row else 'invalid-user-salt')
                    require(row and hmac.compare_digest(calculated,row['password']),'Fel e-postadress eller lösenord.',401);user=row['id']
                token=secrets.token_urlsafe(32);db.execute('INSERT INTO sessions(token,user_id,expires) VALUES(?,?,?)',(hash_token(token),user,now()+604800))
                ...
                result={'ok':True}
                if data.get('native') is True: result['token']=token
                return result,f'vh_session={token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=604800{secure}'
            if path=='/api/campaigns' and method=='GET':
                ids=db.execute("SELECT id FROM campaigns WHERE status='active' AND starts<=? AND ends>? ORDER BY created DESC",(now(),now())).fetchall()
                return {'campaigns':[self.store.campaign(db,r[0]) for r in ids]},None
            match=re.fullmatch('/api/campaigns/([a-f0-9]+)/?',path)
            if match and method=='GET':
                c=self.store.campaign(db,match[1]);require(c['status']=='active','Kampanjen är inte publicerad.',404);return c,None
            ...
            if path=='/api/manage/campaigns' and method=='GET':
                ...
                for r in ids:
                    ...
                    c['completed']=db.execute('SELECT count(*) FROM hunts WHERE campaign_id=? AND completed IS NOT NULL',(r[0],)).fetchone()[0]
            ...
            if path=='/api/manage/campaigns' and method=='POST':
                ...
                days=integer(data.get('voucher_days',14),1,365);stops=data.get('stops')
                require(isinstance(stops,list) and target<=len(stops)<=100,'Lägg till minst lika många platser som insamlingsmålet.')
                locations=[]
                for s in stops:
                    require(isinstance(s,dict),'Ogiltig plats.')
                    locations.append((uid(),cid,text_field(s,'name',2,100),number(s.get('lat'),-90,90),number(s.get('lon'),-180,180),integer(s.get('radius',40),10,100)))
                require(len({(s[3],s[4]) for s in locations})==len(locations),'Varje plats behöver en unik position.')
            ...
            if match:
                ...
                if method=='GET' and not action: return {'hunt':self.store.hunt(db,who,cid)},None
                if method=='POST' and action=='start': return self.store.start(db,who,cid),None
                if method=='POST' and action=='collect': return self.store.collect(db,who,cid,data),None
            if path=='/api/vouchers' and method=='GET':
                rows=db.execute('SELECT v.*,c.title,c.reward,c.venue,c.terms FROM vouchers v JOIN hunts h ON h.id=v.hunt_id JOIN campaigns c ON c.id=h.campaign_id WHERE h.user_id=? ORDER BY v.expires DESC',(who,))
    ...
    def webhook(self,e,raw,event):
        ...
        parts=e.get('HTTP_STRIPE_SIGNATURE','').split(',');values={}
        for part in parts:
            key,_,value=part.partition('=');values.setdefault(key,[]).append(value)
        stamp=values.get('t',['0'])[0]
        require(stamp.isdigit() and abs(now()-int(stamp))<300,'Ogiltig signatur.',400)
        expected=hmac.new(secret.encode(),stamp.encode()+b'.'+raw,hashlib.sha256).hexdigest()
        require(any(hmac.compare_digest(expected,v) for v in values.get('v1',[])),'Ogiltig signatur.',400)
        with self.store.transaction() as db:
            eid=text_field(event,'id')
            if db.execute('SELECT 1 FROM webhook_events WHERE id=?',(eid,)).fetchone(): return {'ok':True}
